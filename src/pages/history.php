...
$name = htmlspecialchars($_SESSION["customers"]);
$result = historyTransaction($name);
...
<head>
    ...
    <title>History of your transaction</title>
...
<body class="bg-slate-100 overflow-x-hidden">
    ...
    <nav id="navside" class="bg-white w-screen md:w-96 h-screen absolute left-[-100vw] md:left-0 top-0 shadow-md px-16 md:p-5 transition-all duration-300 z-20">
        ...
        <ul class="flex flex-col gap-5 mt-10">
            ...
            <li>
                <a href="./history.php" class="flex gap-2 text-xl font-semibold items-center text-white bg-lime-600 justify-center border-2 border-lime-600 rounded-md p-2 shadow hover:bg-lime-600 hover:text-white transition-all duration-300">
    ...
    <main class="md:ml-96 mt-16 md:mt-0 p-2">
        <section class="border-b-2 border-lime-800 pb-2 flex justify-between items-center">
            <h1 class="text-lime-600 font-semibold text-2xl md:text-4xl">Transaction History</h1>
        </section>
        <section>
                ...
                <?php while ($data = $result->fetch_assoc()) : ?>
                    <div class="w-full bg-white mt-2 flex p-2 rounded-md shadow gap-2">
                        ...
                        <div class="grow">
                            ...
                            <p>Rp. <?= $data["price"] ?> x <?= $data["quantity"] ?></p>
                            <p class="font-semibold">Total: <?= $data["total_price"] ?></p>
                            <div class="flex justify-end gap-2 items-center">
                                <a href="./history.php?delete=<?= $data["id"] ?>" class="bg-rose-600 font-semibold px-3 py-1 text-white rounded-md shadow">Delete</a>
                            </div>
                        </div>
                    </div>
                <?php endwhile ?>
            <?php else :?>
                <p class="w-full mt-2 text-slate-300 text-center italic font-semibold">Currently there is no transaction!</p>
            <?php endif ?>
        </section>
    </main>

    <section src="../assets/js/history.js"></section>
</body>
</html>